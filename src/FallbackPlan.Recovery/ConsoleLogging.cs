...
/// <summary>
/// The recovery tool's sink: one writer, one level, no packages (ADR-0043 §1).
/// </summary>
/// <remarks>
/// <para>
/// The service and the CLI share <c>FallbackPlan.Diagnostics</c>, which owns a
/// ring buffer, a rolling file and a redacting renderer. This tool references
/// none of it. Its dependency closure is the smallest that can restore
/// (architecture 11 §2, NFR-PORT-001), and it is the one executable that has to
/// run on a machine that was rebuilt this morning — so forty duplicated lines
/// here are cheaper than a project reference that has to be justified on the
/// worst day somebody has had in a while.
/// </para>
/// <para>
/// Nothing is redacted, because nothing crosses a boundary: this writes to the
/// operator's own terminal, on the machine already holding the files.
/// </para>
/// </remarks>
internal static class ConsoleLogging
{
    ...
    /// <see cref="LogLevel.Warning"/> — the tool's own report is its output, so
    /// what is logged beneath a warning is only what somebody asked to see.
    /// </summary>
