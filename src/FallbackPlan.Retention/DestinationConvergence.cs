...
{
    /// <summary>
    /// Builds the keep filter for one destination's effective policy, or
    /// returns null when the staging archive cannot be walked cleanly — the
    /// caller then converges nothing away and copies whole, because a keep
    /// decision built on a damaged graph would drop objects it merely could
    /// not see.
    /// </summary>
    ...
    /// <returns>The filter, or null to demand the conservative whole copy.</returns>
    public static async ValueTask<Func<string, bool>?> ComputeKeepsAsync(
        IObjectStore store,
