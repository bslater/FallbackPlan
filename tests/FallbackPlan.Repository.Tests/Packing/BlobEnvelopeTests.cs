...
public sealed class BlobEnvelopeTests
{
    ...
    public void BlobEnvelope_ASealedEnvelope_RoundTrips()
    {
        ...
        Assert.AreEqual(FormatLimits.FormatVersion, parsed.FormatVersion);
        Assert.AreEqual(BlobClass.Data, parsed.BlobClass);
        Assert.AreEqual(42UL, parsed.BlobCounter);
    ...
    public void BlobEnvelope_TheShareAndTheShapeDisagreeing_IsRefusedAtConstruction()
    {
        ...
        Assert.ThrowsExactly<ArgumentException>(() => new BlobEnvelope(
            FormatLimits.FormatVersion, BlobClass.Data, new KeyGeneration(0), blobId, salt, 1, writer));
        Assert.ThrowsExactly<ArgumentException>(() => new BlobEnvelope(
            FormatLimits.SymmetricFormatVersion, BlobClass.Data, new KeyGeneration(0), blobId, salt, 1, writer, SealedShare()));
        Assert.ThrowsExactly<ArgumentException>(() => new BlobEnvelope(
            FormatLimits.FormatVersion, BlobClass.Metadata, new KeyGeneration(0), blobId, salt, 1, writer, SealedShare()));
        Assert.ThrowsExactly<ArgumentException>(() => new BlobEnvelope(
            FormatLimits.FormatVersion, BlobClass.Data, new KeyGeneration(0), blobId, salt, 1, writer,
