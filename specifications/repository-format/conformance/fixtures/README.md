...
A complete, tiny **write-only** repository (ADR-0042; format v2,
specification 03 §9): the same deterministic 200 000-byte `fixture.bin` and
the same six-step sequence layout as the v1 fixture, but with **no key
object anywhere** — the descriptor records the sealing public key, the KDF
salt and the parameters, and every key derives from the fixture passphrase
`fallbackplan-fixture-v2-passphrase` (same deliberately small Argon2id
parameters as v1). The data blob's records are sealed under a per-blob
content key wrapped to the repository public key; its footer — the
structure plane — opens under the metadata class key.

**Provenance and the read contract.** The generator is
`tests/FallbackPlan.Repository.ConformanceTests/FixtureRepositoryV2.cs`.
Unlike the v1 fixture there is **no byte-identical regeneration**: sealing
takes a fresh random content key and a fresh ephemeral X25519 share per
blob, by design, so no two generations share bytes. What this committed
copy freezes is the **read contract**, enforced by
...
bundle alone opens the structure plane — record tables, manifests,
catalogue rebuild, journal — and answers `ContentSealed` for content, and
the passphrase-derived authority restores `fixture.bin` byte-identically.
