...
This specification defines versions **2** and **3**; format 1 is withdrawn ([ADR-0014 Amendment 1](../../docs/adr/0014-format-versioning-and-stability.md#amendment-1-2026-09--format-1-withdrawn-before-freeze)). Version 3 changes one thing: a sealed record stops encoding where it lives — its key is scoped to the object rather than the blob, its nonce is carried rather than positional, and its associated data no longer binds the ordinal ([04 §3–§4](04-record.md#3-nonce), [03 §5.4](03-keys.md#54-format-v3-the-key-is-the-records), [05 §2.2](05-blob.md#22-format-v3-data-blobs-the-sealed-record-key); [ADR-0052](../../docs/adr/0052-relocatable-records-format-v3.md)). A version-3 repository lists feature `0x0003` ([01 §3.2](01-object-layout.md#32-body)), and a reader reads every version it implements.

The descriptor is **not** the only statement of a repository's version. It records the version the repository was **created** at and never changes; a signed format-upgrade record ([11 §5](11-lifecycle-objects.md#5-format-upgrade-record)) records that the repository writes a newer one from its next sealed object onward, and the higher of the two is the **effective** version. So one repository may hold objects of two versions, each declaring its own in its envelope, and a reader must implement every version it meets rather than only the descriptor's. Note the consequence for the required-feature refusal above: an upgraded repository's descriptor carries the feature list it was created with, so a reader that does not implement the newer version is not refused at the descriptor — it refuses the first object written at that version instead ([05 §2](05-blob.md#2-cleartext-envelope)).
