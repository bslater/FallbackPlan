...
| 07 | [Retrieval](07-retrieval.md) | An owner reading its own replica back: listing, ranged reads, the owner inventory, and the passphrase-proved **claim** that restores an owner's access after total loss (§5) | Written; implemented except §5's claim |
...
Document 07 is now written and implemented (ADR-0041): an owner opens its own replica over the session — attribution-authorised, read-only, ciphertext both ways — lists and range-reads it, and a hub holding no local content — its staging archive lost, or a direct-ship set for which that is the normal state — restores from a peer over the wire alone; `FallbackPlan.Hosts.Tests` proves that drill end to end between two live services.
