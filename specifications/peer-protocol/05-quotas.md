...
The attribution also carries two published public keys when the source publishes them ([03 §3.1](03-replication.md#31-replicationoffer) keys 4 and 5): the repository's **reclaim public key** ([ADR-0055](../../docs/adr/0055-reclaim-authority.md)) and the installation's **claim public key** ([ADR-0053](../../docs/adr/0053-peer-claim-and-configuration-recovery.md)). A destination holds no repository keys by design, so these recorded keys are the only things it can check against: the first, a deletion instruction's signature ([06 §3](06-retention.md#3-what-the-spoke-validates)); the second, a claim ([03 §6](03-replication.md#6-the-claim)). Each is recorded once and never replaced by a later offer.
...
**The claim is the one exit from that rule on this wire, and it is not re-pairing.** (The other exit is not on the wire at all: the destination's operator re-pointing an attribution that recorded no claim key, through their own command contract — [ADR-0053 Amendment 3](../../docs/adr/0053-peer-claim-and-configuration-recovery.md#amendment-3-2026-09--the-operators-re-attribution-is-a-stated-verb) — which is a judgement on their own machine, not a proof, and is refused wherever a claim key makes the claim possible.) A successful claim ([03 §6](03-replication.md#6-the-claim)) re-points an attribution at the claimant's identity, which is what lets a rebuilt machine have its own replica back. It rests on a signature under the claim public key recorded at first attribution — something the previous machine published and no session since could change — rather than on the fact of a pairing, which anybody can obtain. The rule above stands for every other case. The one thing an attribution's claim key licenses *before* a claim is read from the replica rather than the ledger: the KDF salt and costs in its descriptor are served to a paired claimant so it can derive at all ([03 §6.1](03-replication.md#61-replicationclaimopen-and-replicationclaimparameters)); an attribution recording no claim key contributes nothing to that answer.
