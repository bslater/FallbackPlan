...
| Know what would go red if an invariant broke | [Proof obligations](proof-obligations.md) |
...
- [Shared Bodu Merkle tree](bodu-merkle-requirements.md) — consumer-side requirements on `Bodu.Security.Cryptography` for an RFC 6962 Merkle tree with inclusion proofs, so the tree, path and verification code [ADR-0065](adr/0065-merkle-commitment-and-chunk-possession.md) needed lives in a crypto library rather than in a backup product; **answered upstream** — 0.7.0 ships `MerkleTree` with the RFC's tree shape, inclusion and consistency proofs and the length-bound root, and withdraws the two divergent types the document was written against; **adopted** in slice 32 — the tree, bound root, path and verifier are the library's, the streamed leaf hash stays FallbackPlan's
- [What else Bodu 1.0.0 offers](bodu-1.0.0-survey.md) — the rest of the six packages' public surface against what `src/` hand-rolls: one adoption (the Merkle tree), one thing parked against a named future slice (the rate gate, for NFR-PERF-013's network limit), and one duplication that turned out to be the platform's to serve rather than Bodu's

## Decisions

Status per record. **Sixty-three of the sixty-nine are Accepted.** Among them: 0005, 0006, 0008, 0009, 0011 and 0016–0018 following the [pressure test](review/2026-08-fix-pressure-test.md); 0019–0029 on the evidence recorded in them (0028 amended once implementation decided what "or an equivalent" means on Linux); 0001 — dual AGPL-3.0-only + commercial, with `specifications/` under Apache-2.0 ([LICENSING.md](../LICENSING.md)); 0003, 0010, 0013 and 0030 at the freeze-gate pass (0030 amended once when RFC 7250 proved unreachable on the platform); and 0031–0051 as their slices were built, through the direct-to-destination pair 0046/0047 and the determinate-progress and lifecycle records 0048/0049 to the completed-run drill-down 0050 and the local-placement condition 0051; and 0054/0055, which put the recovery drill on a schedule and split the authority that deletes from the authority that publishes (0020 amended a second time to admit a destination that holds no keys at all); 0056–0059 as their slices landed; and 0060, which withdrew the recovery kit and superseded 0013 — the passphrase is the whole recovery credential; and 0061, which adopts a destination's archives back under their original ids so a rebuilt machine resumes rather than re-seeds; and 0062, which makes the destination the witness for a whole state directory rolled back, so the newest backup is neither deleted nor collided with; and 0063, which gives the peer plane its signed audit record — a destination's deletion receipt under its own device key, verified by the commander and filed by both; and 0064, which carries the same shape to the push — a replication receipt of what the peer committed and holds, the first figure the ledger has ever counted a peer complete on, and one reader over both kinds; and 0065, which puts a Merkle commitment in the index so a peer can be challenged for one leaf of a blob instead of the whole of it, and the leaf's bytes rather than a hash of them are what settle it; and 0066, which lets an existing repository move to a newer format by an appended signed record rather than by a descriptor rewrite no copy would accept; and 0067, the compactor those three format slices were for — a rewrite that moves a blob's live records without holding a key that could open one, and that deletes nothing, leaving the collector to condemn what it drained; and 0068, which stops a restore reading the repository to read a file — the catalogue says where each record is, the blob is opened only when that turns out to be wrong, and neighbouring records are fetched in one ranged read whose first run collects the envelope on its way past; and 0069, the first of NFR-PERF-013's four named limits to exist — hours an operator names in which background work does not start, and in which a capture already running parks through the pause gate 0047 built for preemption rather than through a second suspension written for the purpose.

**Five remain `Proposed`, each for a stated reason rather than by neglect:** 0002 and 0004 await the corpus benchmark and the hash decision ([Q5](open-questions.md#q5--segmentation-default), [Q6](open-questions.md#q6--segment-hash-function)); 0012 awaits a second storage provider to test the contract against; 0014 is provisional by design until the format freezes; 0015 is gated on the legal review in [Q2](open-questions.md#q2--third-party-reader-licence-and-reuse-posture). 0053 and 0052 have left that list: 0053's claim ceremony is built, and its amendment records the two things building it changed, with the set's shape now travelling in the archive itself (0061) rather than in any kit; 0052 was a design taken deliberately early, while making it was still a format revision rather than a data migration, and every plane of it is now built — the record and blob planes with it, the index plane as 0065 — and since 0066 format 3 is what the product creates rather than what a flag asks for.

**A decision's status is not its implementation state**, and the two are tracked separately on purpose: `Status:` says whether the decision was accepted, and [implementation status](implementation-status.md) says whether the code does it. The two can differ in both directions, and do: two of the five records still marked `Proposed` are built and tested, and 0030 is Accepted while the transport that would carry it does not exist.
...
| [0064](adr/0064-replication-receipts.md) | Replication receipts: the destination's signed statement of what a push created and what it holds afterwards, carried in the acknowledgement, verified by the commander, filed by both, counted on the ledger as attested — and both kinds read by one verb, one command and one console card |
| [0065](adr/0065-merkle-commitment-and-chunk-possession.md) | The Merkle commitment and the chunk possession challenge: a sealed blob's index entry commits to an RFC 6962 root over one-mebibyte leaves beside its flat digest, bound to the preimage's length, so a peer can be asked for one leaf and its authentication path instead of the whole blob — and the leaf's bytes, not a hash of them, are what makes the answer a proof |
| [0066](adr/0066-the-format-upgrade-record.md) | The format-upgrade record: a repository moves to a newer format by an appended signed object rather than by a rewritten descriptor — which a destination seeds only if absent, a peer will not replace and a retention instruction may not name — so the move rides every ordinary copy path, takes effect at the next sealed object, and leaves everything already sealed exactly as it is |
| [0067](adr/0067-the-keyless-compactor.md) | The keyless compactor: a blob holding a live minority is rewritten by copying its live records' sealed bytes verbatim into a fresh one, by a compactor holding no key that could open one — which is what format 3 was bought for, and which format 2 cannot do at all, a service holding the structure key and not the content key. The pass publishes supersessions and deletes nothing; the collector condemns the drained blob on its own terms, once every record it held resolves elsewhere |
| [0068](adr/0068-the-catalogue-directed-restore-read.md) | The catalogue-directed restore read: a restore loads nothing and reads each record straight from where the catalogue says it is, falling back to the blob's own footer when that fails, and coalesces neighbouring records into one ranged read whose first run reaches down to collect the envelope. Opening a blob costs three requests before a byte of payload, so no amount of coalescing meets the budget while the read opens the blob — 11 requests over 11 blobs holding 60 records, where the same restore cost 93 |
| [0069](adr/0069-the-background-window.md) | The background window: hours an operator names, in the configuration file, in which background activity does not start — captures, fan-out, the deep sweep and the drills, never a person. A capture caught by the closing window parks through the pause gate 0047 built for preemption, and the pool holds every background run down until the window opens, because a freed worker would otherwise resume the park within milliseconds. The first of NFR-PERF-013's four named limits to exist; the CPU, disk and network limits stay unbuilt and stay named |
| [0070](adr/0070-replica-claim-after-total-loss.md) | Disaster recovery from a peer: the passphrase claims the replica and the device identity does not. Superseded by [ADR-0053](adr/0053-peer-claim-and-configuration-recovery.md) Amendment 2, which reached the same property independently and built it; the record is kept for the reasoning and the renumber it carries |
| [0071](adr/0071-recovering-operation-after-total-loss.md) | Recovering operation after total loss: the repository carries the set's shape, sealed, so what comes back is a working installation rather than a pile of files. Superseded by [ADR-0061](adr/0061-adopt-a-destinations-archives.md), which puts the shape in the policy manifest |
| [0072](adr/0072-snapshot-based-capture.md) | Snapshot-based capture: a privileged helper, and what each platform is actually promised — the two things live capture cannot do, and the honest limit of each operating system's answer |
| [0073](adr/0073-a-browser-suite-for-the-console.md) | A browser suite for the console: the console's pages walked in a real browser, behind an opt-in gate so a machine without one skips rather than fails |
