...
| FR-DR-006 **[new]** | The repository shall carry, per backup set, the set's operational shape — its name, its root labels and the paths they had, its capture rules, its schedule and its retention policy — written on each publication and whenever that configuration changes. | A set whose schedule is edited between backups recovers the edited schedule; a machine holding no configuration reconstructs the set from the repository alone, save for what FR-DR-007 excludes. |
| FR-DR-007 **[new]** | That record shall be sealed to an asymmetric recipient derived from the passphrase, and shall contain no destination in any form — no name, kind, path, endpoint, fingerprint or quota — nor any credential or key material. Destinations shall be recovered from the recovery kit. | A service holding only a write-only bundle can write the record and cannot read it; a repository fixture contains no destination bytes, upholding FR-DEST-006. |
| FR-DR-008 **[new]** | A recovered instance shall resume backing up into the **same** repository its replica belongs to, rather than creating a new one, and shall do so as a new writer identity. | After recovery, the destination holds one repository for the set, not two; the first fan-out transfers only objects the destination lacks; the new writer's journal sequence is gapless from its own start and raises no cloning alarm. |
