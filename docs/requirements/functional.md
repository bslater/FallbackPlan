...
| ID | Requirement | Acceptance |
|----|-------------|-----------|
| FR-USR-001 **[new]** | An installation shall hold named user accounts, the first captured during first-run setup before setup completes. A client acting against a service shall identify itself as one of those accounts before issuing any command other than reading the service's description or logging in. | A fresh installation cannot complete setup without a first account; a second console connecting to a set-up service can render a login screen and nothing else until it authenticates; every other verb is refused by name with how to log in. |
