...
**NFR-PERF-005 is cited in part, and the part it does not cover is named.** The manifest plane is incremental: a manifest whose object the index already locates is not written again, so a one-file backup of a 1 024-file tree rewrites four records rather than every directory in the repository. The per-snapshot **source-identity hint** ([06 §11](../../specifications/repository-format/06-manifests.md#11-source-identity)) is not — it describes the whole snapshot, at roughly 52 bytes per file, every run. That buys rename lineage surviving a catalogue rebuild and it costs a term proportional to repository size, and the trade has not been decided. It is [Q21](../open-questions.md#q21--the-source-identity-hint-is-per-snapshot-and-whole-tree).
...
| NFR-PERF-005 | [02 §6.3](../architecture/02-repository-format.md#63-sharding-and-encoding) | — | `Repository.Tests/IncrementalBackupTests` *(manifest plane only; the source-identity hint is per-snapshot and whole-tree — see the note below)* | 1 |
