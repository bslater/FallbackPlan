...
| NFR-SEC-003 **[amended]** | Nonce uniqueness shall be structural in whichever construction the repository's format uses, per [`../architecture/03-crypto.md` §3](../architecture/03-crypto.md#3-nonce-and-key-construction). **Format 2:** a fresh CSPRNG salt per blob, a key derived per blob, and the record ordinal as nonce. **Format 3:** a key scoped to the object and a nonce drawn per record from the CSPRNG and carried in the record ([ADR-0052](../adr/0052-relocatable-records-format-v3.md)), because a record no longer has a position to be unique by. | Property test: no `(key, nonce)` pair repeats across any generated sequence, in either construction. Format 2: resume produces byte-identical blobs; restart produces a different salt; *N* concurrent writers produce pairwise-distinct salts. Format 3: distinct objects derive distinct keys under one class key, **and one object sealed many times never repeats a pair** — the half a fixed nonce would fail. |
...
| NFR-SEC-009 **[amended]** | Raw or unsealed key material shall never cross the command surface in either direction, and unlocked key material shall be confined to the service account. Operations that mint new access shall re-derive from a user-supplied passphrase per invocation. The sole permitted transit shape is an envelope sealed end-to-end to the service's published recipient key, as a hex string, on the named provisioning and restore-source verbs ([ADR-0042](../adr/0042-write-only-repositories.md) §6). | No command or event carries raw key material; the envelope fields are string-typed, enumerable, and confined to the named verbs by test. Holding a running service is not sufficient to derive any read authority ([T-19](../threat-model.md), [ADR-0028](../adr/0028-service-boundary-and-deployment-topologies.md) §9, [ADR-0060](../adr/0060-the-passphrase-is-the-recovery-credential.md)). |
| NFR-SEC-010 **[new]** | A write-only repository's write bundle shall be one-way: possession of the service's complete state shall yield neither the passphrase, nor the derivation root, nor the sealing private key, nor any key outside the bundle. | Every derived key is an independent one-way KDF output; a documented threat-model entry states exactly what stolen service state yields and what it provably cannot ([ADR-0042](../adr/0042-write-only-repositories.md) §9). |
| NFR-SEC-011 **[new]** | A passphrase captured during first-run setup shall be derived in the client's own process and shall reach the service only as an envelope sealed to that service's published recipient key. Setup shall be available to a local caller only. | No setup command or result carries a passphrase in any form; the envelope field is a hex string on a named verb enumerated by test; the same verb from a paired remote caller is refused ([ADR-0044](../adr/0044-first-run-setup.md) §4, §5). |
| NFR-SEC-012 **[new]** | Account credentials shall live in the service's own state directory with owner-only permissions, written atomically, and shall never enter the repository metadata plane. Verification cost shall be paid once per session, not once per command. | The credential file is readable by nobody but the service account; no destination or replica ever receives it; a session's commands do not each pay a memory-hard derivation, and a connection that never authenticates pays none. |
| NFR-PRIV-001 **[amended]** | No telemetry shall leave the device without explicit opt-in. | Two halves, because neither is sufficient alone. **The build contains no means**: no shipped assembly references an HTTP client; outbound capability is confined to the five assemblies that are the peer protocol and the loopback IPC, each named with what it is for; nothing attaches a `MeterListener` or an `ActivityListener` to the in-box instruments, which is how telemetry would leave with no dependency appearing anywhere; and the set of packages the product ships is pinned whole, so it cannot grow in silence. **And a default run is captured**: a default installation set up, backing a source tree up to a local path, asked for its status and snapshots, restoring a file and planning a retention pass — all through the transport an operator's terminal and the console use — reaches no address on an IP network, resolves no host name and makes no HTTP request, observed by the runtime's own network instrumentation while it happens. The capture sees this process, not the machine: it is not a packet capture and cannot see a child process, which is why the build-level half is what makes one observed run worth generalising from. |
...
| NFR-COMP-004 **[amended]** | The format and conformance fixtures shall be documented sufficiently for an independent restore implementation, for **every** format version the product writes. | A reader written from the specification alone, by an author who did not write the format, in a different language, passes the fixtures. There is a committed fixture repository per writable version — `fixture-repository-v2` and `fixture-repository-v3` — each freezing a read contract rather than bytes, since sealing is randomised by design. This gates the format freeze, and the gate reads against every version a repository may hold: format 3 is what the product creates, format 2 is what every repository created before it is, and after a format upgrade ([ADR-0066](../adr/0066-the-format-upgrade-record.md)) one repository holds objects of both, so a reader that implements only one restores only part of it ([ADR-0014](../adr/0014-format-versioning-and-stability.md)). |
| NFR-COMP-005 **[amended]** | Provider capabilities shall not leak into snapshot or file-version semantics. A capability the engine depends on shall be **checked** rather than assumed: a store declaring it absent is refused by name at the boundary, not discovered deep in a publication ([ADR-0012](../adr/0012-storage-provider-contract.md)). | A repository written to one provider restores identically from another — **which cannot be run today and is stated as owed rather than implied**: there is one provider, so the criterion has no second side and `Storage.ContractTests` proves portability against a reusable suite rather than against a provider that differs. What can be run, and is: a store declaring no conditional create or no ranged reads is refused before it is used, and the refusal names the capability and what depends on it. |
...
| NFR-OPS-002 **[amended]** | Status shall distinguish `never backed up`, `captured`, `protected`, `verified`, `degraded`, and `unrecoverable` — the exhaustive derived vocabulary ([10 §1.1](../architecture/10-observability.md#11-states-must-be-distinguishable); `replicated` and `policy-compliant` retired as never emitted). A glance-level surface may group derived states under fewer words only where no such distinction is crossed, with the derived state one interaction away **[amended]**. | No UI surface merges `degraded` with `unrecoverable`, or `captured` with `protected` — in particular `captured` never reads as the same glance word as `protected`. The console's map carries exactly the derived vocabulary, no dead entries. |
...
| NFR-OPS-005 **[amended]** | Recovery shall be possible from a clean machine using only the repository, the recovery software, and the passphrase ([ADR-0060](../adr/0060-the-passphrase-is-the-recovery-credential.md)). | End-to-end test on a machine with no prior state. |
...
| NFR-OPS-007 **[new]** | Every shipped assembly shall emit its diagnostics through one logging abstraction, with a stable event identifier per message and no logging provider or sink package in any library. Diagnostics shall never be the channel that carries operator-actionable state. | No library references a logging provider package; every message carries an event id unique across the solution; a disabled level allocates nothing; no condition requiring a person is reported only as a log record (NFR-OPS-002 and the [10 §3.1](../architecture/10-observability.md#31-how-a-client-learns-any-of-this) channels still hold). |
| NFR-OPS-008 **[new]** | State the service keeps on its own disk, beside the backups themselves, shall be bounded by a **stated rule** rather than by uptime. Where a rule discards a record, it shall be written down where the record is promised — a bound nobody documented is indistinguishable from a leak, and a bound nobody stated is indistinguishable from a loss. | The peer receipts ([ADR-0063](../adr/0063-deletion-receipts.md), [ADR-0064](../adr/0064-replication-receipts.md)) and the replication spool ([ADR-0057](../adr/0057-resumable-object-transfer.md)) each name their rule and apply it without being asked. Three properties hold of the receipts' rule and are tested: a pile that has grown unreadable is still bounded, because the rule reads file names and never contents; a state directory that was copied or restored still ages correctly, because the age comes from the issue time the name carries and not from a filesystem stamp the copy rewrote; and a pair that has gone quiet keeps its newest few whatever their age, so a bound never empties an audit trail. A listing of such state is priced by what was asked for rather than by what has accumulated. |
...
| NFR-PORT-005 **[new]** | The engine shall not depend on the freshness of a store's listing for correctness. Where a provider does not promise that a listing reflects what it holds, an operation that reasons from an object's **absence** shall refuse rather than proceed, by name and with the promise it is refusing on. | Against a store declaring eventual listing consistency: a collection pass reports the plan it would have acted on and condemns nothing, and a destination's keep-set is refused rather than built — so nothing is deleted on the strength of a view that was incomplete. Operations that reason from an object's **presence** are unaffected and are expected to keep working: a replication pass re-offers what it already sent and the conditional put refuses it, a writer's stale head collides instead of overwriting, and a delta the listing has not caught up to is reported as an unresolved gap rather than assumed away. A pile that has grown unreadable, a state directory copied between machines, and a provider that makes no statement at all are each covered by the same rule. |
