...
>
> **Resolved (2026-08) — the honesty half.** `file_versions.has_alternate_streams` (catalogue schema v5) is recorded by the live projection, the catalogue rebuilder and the forensic rebuilder alike; `RestorePlanner` declares an `alternate-streams` degradation, presence-gated like the symlink one; and the receipt reports the item `degraded` (schema v3) with the aggregate reduced to `Partial` — it also turned out the deferral understated the receipt half, since nothing consumed `plan.Degradations` at all, so a planner-only change could never have stopped `complete`. Held by `RestorePlanTests.RestorePlan_AFileCarryingAlternateStreams_DeclaresTheDegradation` and `…ReportsTheDegradedOutcomeNotComplete`, plus the golden receipt fixture in `RestoreBreadthTests`. The **write-back half stays owed** — the target capability is `false` on every platform until it lands, which is the polarity that keeps the degradation honest.
...
**What was pinned for a decision (SF-1).** One unopenable blob under `blobs/` — a torn orphan nothing references — made the plain cold reader (`RepositoryReader.LoadBlobsAsync`) refuse the entire load, so every plain-path restore was blocked until something removed the orphan, and nothing collects yet. Loud and never wrong bytes, which is the error posture — but the operational consequence on a provider without atomic puts is that a single torn write degrades all plain restores until phase-4 GC. Three choices were pinned: tolerate-and-scope in `LoadBlobsAsync`, sweep torn orphans, or accept until the collector exists.

> **Resolved (2026-08) — tolerate-and-scope, as proposed.** `LoadBlobsAsync` now skips a blob that fails to open with a `BlobFormatException` and reports it in a new `RepositoryReader.SkippedBlobs` list — key and reason, structured, never silent — converging the plain reader on the posture the recovery tool (`RecoverySession`, "a damaged blob is skipped with a note; corruption is local, 04 §7") and the verify engine already held. A skipped blob's records read as absent, which every downstream caller already refuses loudly, and naming damage exhaustively remains `verify`'s job; an `IOException` still propagates, because a transient store fault is not a damage finding. The torn-write assertion flipped to a restore as promised, and five prior pins of the whole-load refusal (`CorruptionHarnessTests` envelope and footer, `ArchiveCorruptionTests` truncated locator, trailing bytes, foreign repository) flipped to the scoped shape with it. Committed snapshots now restore through the plain path past a torn orphan, held by `StoreFaultTests.Publication_ABlobPutTears_FailsTheJobAndTheForensicPathStillRestoresTheCommittedSnapshot`.
