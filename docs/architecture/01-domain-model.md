...
| **Format v3** | The repository format in which a sealed record stops encoding where it lives: its key is derived from the object rather than the blob, its nonce is drawn per record and carried in the record, and its associated data names no position — so the sealed bytes can be copied into another blob and still open ([ADR-0052](../adr/0052-relocatable-records-format-v3.md)). Created only on request (`init --format-version 3`); format 2 remains the default and the only format the service creates. A repository's version is fixed at creation and declared by a required descriptor feature, so a reader that does not implement it refuses by name. Note that a repository's version and the version stamped on one of its blobs are different numbers — see `Domain/FormatVersions`. |
