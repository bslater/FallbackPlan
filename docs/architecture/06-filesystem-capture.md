...
> **What this needs, and therefore does not always get.** The rewrite depends on the catalogue to resolve where the prior manifest lives, so it holds exactly where identity-based rename detection holds: with a warm catalogue. After a rebuild the hints still recover the *ancestry*, but the content is read again, because a hint names an object and not its location. Recovering the location too would mean a store listing, which is the cost the hint layout exists to avoid.
