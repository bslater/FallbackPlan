...
**A renamed file still gets a new manifest — but not a new read.** A manifest states its own name, so re-emitting the prior object under a new tree entry would produce a tree that says one name and a manifest that says another. The new manifest carries the new name and names the prior version as its parent. Everything else in it is *inherited*: the prior manifest is fetched through a targeted read — the catalogue resolves the object's location, and one blob is opened through its recovery footer, so the cost is a handful of range reads rather than a listing — and rewritten with the new name, the new normalisation, the current metadata, and the ancestry. The segment references, the whole-file hash, the length and the sparse extents all carry over unchanged, because they describe bytes nothing re-examined and must not be re-stated as though they had been.

### 4.3 Unchanged bytes are not an unchanged file

The reuse short-circuit is keyed on identity, size and modification time, and that answers the question *did the content change*. It is not the same question as *did anything about this file change*, and treating it as though it were discards a whole class of edit: POSIX `chmod` moves ctime and not mtime, so a file whose permissions were just tightened presents an identical identity, size and mtime. Re-emitting the prior version there republishes the old mode, owner, group and extended attributes, and the loss is invisible until somebody restores. Somebody who notices a private key is world-readable, fixes it, and takes a backup is owed a backup of the file as it now is.

So the catalogue records a **metadata digest** beside the other reuse inputs — canonical CBOR over the metadata map, hashed — and the short-circuit fires only when the content signals *and* the digest agree. When the bytes match but the digest does not, the file takes the same inherited path a rename takes: the prior manifest is fetched and rewritten with the current metadata. A new version, a real ancestry, and not one byte of content re-read.

**Access time is excluded from that digest, and only access time.** Reading a file moves its atime, and a backup's own read is a read — including it would make every file differ from itself on the next pass and turn whole-tree reuse into whole-tree re-capture, which is the cost [NFR-PERF-003](../requirements/non-functional.md) exists to avoid. Atime is still captured and still restored; it is simply not evidence that anything changed, because it says something about the observer rather than about the file. That is the sole exception, and it was found by an agent-pass test dropping from "2 unchanged" to "0 unchanged" rather than by inspection.

That read is best-effort by construction. If the prior manifest cannot be fetched, authenticated, or decoded, the file is captured normally — which is what would have happened anyway — and nothing is reported as damage: a blob this backup does not otherwise depend on is `verify`'s business, and failing a backup over an object nobody asked for would be the worse outcome.
...
> **What this needs, and therefore does not always get.** The rewrite needs a prior version that is provably the same bytes, and that proof rests on identity, which a rebuilt catalogue does not have. So after a rebuild the hints still recover the *ancestry* — the new version names its parent — but the content is read again, because nothing available can rule out that the file changed while the cache was cold. This is the same boundary the unchanged-file short-circuit has, for the same reason, and it is a deliberate refusal to guess rather than a missing capability.
