...
> **Amended by [ADR-0051](../adr/0051-local-destination-placement.md) (2026-08).** The boundary `protected` asks about moved from the machine to the **volume**: a `same-machine` destination — a second drive — now earns `protected`, with its residual risk (drive failure yes; theft, fire, machine loss no) named beside the badge, and drive separation became the condition of *choosing* a local destination in the first place. Only `same-volume` still caps at `captured`, and it can no longer be newly chosen — so PT-8's same-disk false confidence stays impossible from both ends.
