...
| A snapshot commits globally, across all destinations | One offline destination then stalls all protection, so a laptop away from its home peer is unprotectable exactly when backup matters | [C5](review/2026-08-architecture-review.md#c5--snapshot-commit-is-defined-so-that-one-offline-destination-stalls-all-protection) → [ADR-0011](adr/0011-commit-versus-replication-semantics.md) |
