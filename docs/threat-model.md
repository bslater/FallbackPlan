...
**Mitigation:** the catalogue retains the highest observed generation and per-writer sequence in durable local state; deltas form gapless per-writer chains so a missing sequence is *detected* rather than assumed absent; when that local state is itself lost or rolled back, the repository's own signed checkpoints, deltas and journal keys are the witness at archive open ([ADR-0008](adr/0008-index-generations-and-checkpoints.md)); and when the *whole* state directory rolls back — taking a direct-ship set's metadata plane with it — the destination is the witness: every fan-out pass reads the destination's journal head for this writer, moves the sequence past it, deletes nothing there, and heals the local metadata and catalogue from the destination ([ADR-0062](adr/0062-the-destination-is-the-rollback-witness.md), FR-DEST-018). [`03-crypto.md` §6](architecture/03-crypto.md#6-authentication-of-repository-state).

**Residual:** a peer destination is not yet asked, so a set whose only destination is a peer has no witness beyond the colliding-put refusal; and a rollback that restores the state directory and every destination from one older image has no witness anywhere, because nothing is ahead of anything.
...
**Residual:** a destination keeps no signed record of what it deleted, only the count it acknowledges, so the repository plane has an audit artefact (the tombstone) and the peer plane has none. FR-GC-008 records it as not met.
