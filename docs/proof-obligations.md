...
| Memory is bounded by configuration, not by the size of the input | One synthetic file far larger than memory; measure peak RSS | `Repository.Tests/BoundedMemoryTests`, `PerformanceTests/MemoryBoundProof` | **Partly proved** — the scaling property is held at test scale, and nothing gates peak RSS in CI. `Repository.Packing/BlobWriter` promotes a finished spool by reading it whole, which at `Domain/FormatLimits`' maximum blob size is twice NFR-PERF-001's budget (FR-ARCH-002, NFR-PERF-001) |
...
| A sealed piece can be relocated between containers without its key | Move a record into another blob and read it back with the same key material | — | **Open by decision** — it cannot. The nonce is the record's position within its blob and the AAD binds it, so compaction decrypts and re-seals, which [ADR-0025](adr/0025-compaction-reseals-records.md) states and accepts. Reopened as the format v3 arc; nothing compacts yet, so the cost of reversing it is a format revision rather than a data migration |
