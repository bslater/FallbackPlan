...
| Invariant | What would falsify it | Proof | State |
|-----------|-----------------------|-------|-------|
| `verified` means bytes were read at the destination and checked against evidence it does not hold | Corrupt a blob at a set's only destination, and separately let a converged set sit with nothing to copy | `Hosts.Tests/DestinationVerificationTests`, `Hosts.Tests/DirectShipVerificationTests`, `Replication.Tests/ReplicaVerifierTests` | **Proved** — a peer answers a keyed range challenge; a readable destination has its sampled blobs *opened* at the replica, so the AEAD tag the writer computed is the evidence and no second copy is needed. Challenges are due on the proof's age, not on a copy being due (FR-VER-001, FR-VER-002) |
