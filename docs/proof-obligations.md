...
| A machine that lost everything can claim its peer replica back | Delete the state directory and the archive, then install fresh and try to recover | `Hosts.Tests/PeerClaimTests`, `Repository.Tests/ClaimFromKitTests`, `Application.Tests/ReplicaOwnerStoreTests` | **Partly proved** (FR-REP-001, FR-KIT-006) — the drill destroys the archive, the configuration, the state directory and the device keypair, and the replica comes back: a fresh install pairs afresh, claims under a key derived from the printed kit and the passphrase alone, and reads the replica on the very request that was refused before the claim ([ADR-0053](adr/0053-peer-claim-and-configuration-recovery.md) Amendment 1). Building it found §1 unreachable as written — it derived the claim key from the repository's own hierarchy, and a claimant that has lost the repository holds an installation kit that names no repository and carries no key object — so the key is derived from the **installation** instead, and the record is amended rather than worked around. Two parts are not proved because they are not true. A replica attributed before the claim key existed has no key to check against: it self-heals on one more offer from an updated source, and is otherwise the destination operator's to re-point, which is a verb ADR-0053 §3 names and nothing implements. And the set's own shape still lives only in the local configuration, which is §4, still open |
