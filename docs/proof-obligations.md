...
| The recovery kit and the passphrase are sufficient — no catalogue, no local state, no this machine | Destroy the state directory, the catalogue and the source, then restore from the destination alone | `Repository.Tests/InstallationKitDrillTests`, `Hosts.Tests/RecoveryHostTests`, `eng/recovery-drill.sh` | **Proved** (FR-KIT-003, FR-KIT-006) |
