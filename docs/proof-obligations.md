...
| A machine that lost everything can claim its peer replica back | Delete the state directory and the archive, then install fresh and try to recover | `Hosts.Tests/PeerClaimTests` — which runs the falsifier and pins the refusal | **Unproved** (FR-REP-001, FR-KIT-006) — the drill now exists and the invariant does not hold: a fresh install, paired afresh with the peer, is refused its own repository on attribution. Every other peer drill preserves the state directory, which is what a dead machine does not, so this one destroys it. [ADR-0053](adr/0053-peer-claim-and-configuration-recovery.md) decides the ceremony that would let it; nothing implements it. Building it found that the record's §1 cannot be reached as written — it derives the claim key from *the repository's own hierarchy*, and a claimant that has lost the repository holds an installation kit that names no repository and carries no key object, so there is nothing to derive from. The decision is reopened rather than worked around. The set's own shape still lives only in the local configuration |
