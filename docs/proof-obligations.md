...
| A machine that lost everything can claim its peer replica back | Delete the state directory and the archive, then install fresh and try to recover | `Hosts.Tests/PeerClaimTests`, `Protocol.Tests/ReplicationMessageTests`, `Application.Tests/ReplicaOwnerStoreTests` | **Partly proved** (FR-REP-001, FR-WOR-002) — the drill destroys the archive, the configuration, the state directory, the installation credential and the device keypair, and the replica comes back: a fresh install pairs afresh, is served the KDF salts and costs behind the peer's claimable replicas, claims under keys derived from the passphrase and nothing else, and reads the replica on the very request that was refused before the claim ([ADR-0053](adr/0053-peer-claim-and-configuration-recovery.md) Amendments 1 and 2). Building it found §1 unreachable as written — it derived the claim key from the repository's own hierarchy, and a claimant that has lost the repository held, at the time, an installation kit that named no repository and carried no key object — so the key is derived from the **installation** instead, and the record is amended rather than worked around. Two parts are not proved because they are not true. A replica attributed before the claim key existed has no key to check against: it self-heals on one more offer from an updated source, and is otherwise the destination operator's to re-point, which is a verb ADR-0053 §3 names and nothing implements. And the set's own shape still lives only in the local configuration: §4 closed as will-not-do with the kit, and the set is re-declared after a rebuild until add-destination-then-adopt exists |
...
| A peer replica is drilled, not only challenged | Let a peer-held set's read path rot unexercised | — | **Unproved** (FR-DRL-002) — only local-path destinations drill. A peer's replica is behind the wire, and restoring from one on a cadence the peer never agreed to is peer-protocol work rather than a schedule ([ADR-0054](adr/0054-scheduled-restore-drills.md) §6). The possession challenge still proves the bytes are there |
