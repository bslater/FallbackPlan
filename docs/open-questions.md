...
**Decided** (2026-08, [ADR-0044](adr/0044-first-run-setup.md) §6): **a floor of `Passphrase.RecommendedMinimumLength` together with a small documented strength estimate**, enforced **at the setup boundary only**. The number was twelve at decision; ADR-0044's second amendment later raised it to **sixteen with composition rules** (an uppercase letter, two digits, a special character), keeping the boundary-only scoping this answer rests on.
...
**Owner:** maintainer · **Blocks:** arming disaster recovery for format-v2 sets only; v1 is unaffected

[ADR-0070](adr/0070-replica-claim-after-total-loss.md) arms disaster recovery by having a source register a passphrase-derived public key with each destination, during replication ([peer-protocol 03 §3.2.1](../specifications/peer-protocol/03-replication.md#321-registering-the-claim-credential)). That works for a v1 set, whose service holds the passphrase and can derive the credential whenever a backup runs.
...
| Does an offline destination block snapshots? | For a staging set, no — commit is per-replica ([ADR-0011](adr/0011-commit-versus-replication-semantics.md)). For a direct-ship set, one offline destination still does not block (the run writes to the reachable ones), but with **no** reachable destination the capture deliberately refuses — the owner's accepted trade for holding no local copy ([ADR-0046](adr/0046-direct-to-destination-publication.md) §4, [ADR-0011 Amendment 4](adr/0011-commit-versus-replication-semantics.md#amendment-4-2026-08--commit-re-unifies-with-the-destinations-for-direct-ship-sets)) |
...
| What is in the recovery kit? | Withdrawn ([ADR-0060](adr/0060-the-passphrase-is-the-recovery-credential.md)) — there is no kit; the passphrase and reach to an archive are the whole recovery credential |
...
| Can compaction relocate records byte-identically? | In format 2, no — compaction decrypts and re-seals; the AAD and its ordinal are unchanged ([ADR-0025](adr/0025-compaction-reseals-records.md)). In format 3, yes, and it is built: the key is the object's, the nonce rides the record and the AAD names no position, so `Repository.Packing/BlobWriter`'s `AppendSealedRecordAsync` copies a sealed record into another blob verbatim ([ADR-0052](adr/0052-relocatable-records-format-v3.md)). The compactor written over it is built for format 3 and refuses format 2 by name — where the answer is not merely "decrypt and re-seal" but "a service cannot", holding the structure key and not the content key ([ADR-0067](adr/0067-the-keyless-compactor.md)) |
