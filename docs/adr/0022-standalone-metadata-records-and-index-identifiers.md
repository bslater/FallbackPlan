...
> **Practice note (2026-08, base-hardening round).** The **standalone snapshot object** joined the hint pattern: it seals under its write intent's sequence rather than drawing a number of its own. `/snapshots/…` is not a sequence-addressed key, so an own number satisfied none of the four cases above and surfaced as one junk void delta per publication, discharged by the following run, forever. The arithmetic argument for hints applies verbatim, and the cryptographic argument is unchanged. The same round added the writer-side bookkeeping case 4 implies: a blob's counter is marked accounted the moment its put is acknowledged under a live intent, so no later run voids a number a durable blob embeds. Held by `InterruptionTests/SequenceAccountingTests`.
