...
**Built:** the record, blob and index planes, and the compactor they were for ([ADR-0067](0067-the-keyless-compactor.md)). `Domain/FormatVersions` (the questions the code asks of a version, and the container stamp a repository version implies — which is not the same number), `Domain/FormatLimits` (`LatestFormatVersion` beside the creation default), `Repository.Format/Descriptor/RepositoryDescriptorCodec` (feature `0x0003` and the consistency rule that a format-3 descriptor must list it and a format-2 one must not), `Repository.Crypto/RecordKeyDeriver` (the object-scoped key, and the writer's seed derivation beside it), `Repository.Format/Records/RecordAad` (the 51-byte relocatable layout) and `RecordNonce` (the carried random draw), `Repository.Packing/SealedRecordKey` (the share pinned to the repository and the object), `Repository.Packing/RecordFraming` (the one arithmetic every length site asks), `Repository.Packing/SealedContentKeyOpener` (a grant answering per blob and per record from one scalar), `Repository.Packing/BlobWriter` (the format-3 append, `AppendSealedRecordAsync` as the relocation primitive, and the resume walk under the checkpointed seed), `Repository.Packing/BlobReader` (three keyings decided by the envelope, a refused share contained to its record), `Repository/ArchiveSession` and `Repository/ManifestBuilder` (the descriptor's version reaching the containers they stamp), `Cli/CliApplication` (`init --format-version`); the index plane is [ADR-0065](0065-merkle-commitment-and-chunk-possession.md)'s — `Repository.Index/IndexDeltaCodec` (delta key 11) and `Repository.Catalogue/Catalogue` (schema 7's `merkle_root`), published only at format 3 or above. `Repository.Tests/Crypto/RecordKeyDeriverTests`, `Repository.Tests/Crypto/RecordCipherTests`, `Repository.Tests/Crypto/NonceUniquenessTests`, `Repository.Tests/Format/RecordAadTests`, `Repository.Tests/Packing/RecordFramingTests`, `Repository.Tests/Packing/SealedRecordKeyTests`, `Repository.Tests/Packing/RelocatableBlobTests`, `Repository.Tests/EndToEnd/WriteOnlyRepositoryTests`, `Repository.ConformanceTests/FixtureRepositoryV3Tests`, `Repository.ConformanceTests/RecordCipherConformanceTests`, `Repository.ConformanceTests/RecordFramingConformanceTests`, `InterruptionTests/WriteOnlySpoolResumeTests`, `Cli.Tests/WriteOnlyCommandTests`.
...
> **It closed as intended (2026-09).** The reversal landed here and cost a
> format revision; the compactor landed afterwards as
> [ADR-0067](0067-the-keyless-compactor.md) and cost no migration, because
> format-2 repositories are read in place and nothing was rewritten. The
> paragraph above is left as it was written, because it is the argument the
> outcome tested.
...
4. **A digest a peer can be challenged against.** *(Added 2026-09.)* The
   ...
   > **Closed (2026-09) — [ADR-0065](0065-merkle-commitment-and-chunk-possession.md).**
   > The root sits **beside** the flat digest and never replaces it, the
   > leaf size is **fixed by the format** at one mebibyte, and the root
   > binds the preimage's length — which plain RFC 6962 does not, and
   > without which a destination could understate its copy and exempt its
   > last leaf from ever being drawn. Delta key 11, published only at
   > format 3 or above; the challenge over it is peer-protocol 07 §3.6's.
...
| 2026-09 | Accepted (the compactor built over it) | The primitive this record exists for has its consumer: [ADR-0067](0067-the-keyless-compactor.md) rewrites a blob holding a live minority by copying its live records' sealed bytes verbatim through `AppendSealedRecordAsync`, holding no key that could open one, and [ADR-0025](0025-compaction-reseals-records.md)'s twelve exit criteria are answered. Nothing compacts a format-2 repository and nothing will — re-sealing there needs a content key the service does not hold — so the remedy is the append-only upgrade |
