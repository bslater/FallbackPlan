# ADR-0070 — Disaster recovery from a peer: the passphrase claims the replica, and the device identity does not

**Status:** Superseded by [ADR-0053](0053-peer-claim-and-configuration-recovery.md) Amendment 2 (2026-09) — the passphrase claims the replica and the device identity does not, reached and built on the branch this merge brings in
**Date:** 2026-08
**Requirements:** FR-DR-005, NFR-SEC-009 — the other disaster-recovery rows this record proposed were not carried across the merge that renumbered it; see the status history.
**Related:** [ADR-0010](0010-local-store-separation.md), [ADR-0013](0013-recovery-kit.md), [ADR-0030](0030-peer-identity-and-pairing.md), [ADR-0034](0034-hub-and-spoke-destinations.md), [ADR-0041](0041-guided-restore-and-peer-retrieval.md), [ADR-0042](0042-write-only-repositories.md), [ADR-0071](0071-recovering-operation-after-total-loss.md), [peer-protocol 05](../../specifications/peer-protocol/05-quotas.md), [peer-protocol 07](../../specifications/peer-protocol/07-retrieval.md)
...
- **This record recovers data and not operation, and that half is
  [ADR-0071](0071-recovering-operation-after-total-loss.md).** A machine that
...
- **A write-only (v2) set cannot arm itself unattended, and this is a real
  ...
  and one of them must be picked before this record's recovery path can be claimed for v2. v1 is
...
| 2026-08 | Proposed | Built to the wire and onto the contract. The destination offers its token with the final inventory page and stores the credential a source registers against it; a claimant derives the private half per candidate and signs. Contract **1.17** adds `claim_replicas`, carrying the Argon2id root sealed to the service's recipient key — the fourth verb permitted a sealed envelope, and the reason it could not be avoided is that a rebuilt machine has no repository to read the KDF salt from, so the derivation happens on the client with the recovery kit. The verb names a *pairing* rather than a destination, because configuration is one of the things a claim exists to recover, and defaults to every pairing: a household that has just re-paired need not know which friend holds what. An unreachable peer is reported beside the others rather than aborting them — a claim runs when the far end is least likely to be awake |
| 2026-09 | Superseded | Renumbered from ADR-0046 in the merge that brought this line together with the branch, which had already assigned 0046 to a different decision. Superseded rather than merged: the property recorded here — that the passphrase claims the replica and the device identity does not — was reached independently on the branch and built there under [ADR-0053](0053-peer-claim-and-configuration-recovery.md) Amendment 2, with end-to-end drills. The implementation described above is removed rather than carried across, and the contract verb it added is withdrawn at its original number. |
