...
| 2026-09 | Built (second asker) | The pause gate acquires a second asker it was not designed for, and the fit is exact: a **background window** closing over a running capture parks it through `PauseGate`, and a closure outlasting `_maxPause` self-cancels the run into the interruption-safe re-run path — the behaviour that parameter's own doc comment describes, reused whole ([ADR-0069](0069-the-background-window.md)). Two things in `JobScheduler` moved to make it hold: the hold is a **standing state of the pool** rather than a per-job ask, because `TryTakeWriterWork` resumes the best-ranked parked run the moment a worker frees and would otherwise undo the park within milliseconds; and `Pause` now carries its reason, because this record hard-coded *"suspended for a higher-priority run"* into the journal row a park writes, and with a second asker that sentence — the one a person reads the next morning — was about to be wrong half the time. First ask wins it, so a window closing over an already-preempted run does not rewrite what happened |
