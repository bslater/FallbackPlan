...
Everything the row names is held by tests, including the 04 §5.1 kill matrix through a two-destination sink (`Hosts.Tests/DirectShipFaultSweepTests`). What keeps the row at **Partly built** is deliberate flag-gating, not doubt: `direct_ship` stays opt-in until the peer write adapter lands (a declared peer is a stated `NotSupported` ledger row today), a retention-with-trimming drill runs over aged direct-ship snapshots, and the default flips — the [phase-2 plan's pickup list](phase-2-execution-plan.md#where-to-pick-up) leads with exactly this tail.
