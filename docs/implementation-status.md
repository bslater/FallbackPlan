...
What is left is the remainder of Amendment 2 — termination notices and the `Revoked` refusal, terms enforcement (spec 05) — plus destination verification (spec 04), hub-planned retention against a spoke (spec 06), and the console features gated on the two open questions of ADR-0028 — streaming restored content to the operator (Q18) and per-operator identity on a shared console (Q19).
...
Still ahead: the `sync` verb superseding `replicate`, termination notices, quotas, retention — sequenced in the [roadmap](roadmap.md). The negotiated pairing role landed with the peer slice — see [0030](#0030--the-socket-exists).
...
| [Hub-and-spoke arc](roadmap.md#the-hub-and-spoke-arc--multi-destination-backup-sets-current) | In progress ([ADR-0034](adr/0034-hub-and-spoke-destinations.md)): configuration schema v2, per-set staging archives, local-path fan-out and the status matrix built; peer destinations next — see [0034](#0034--sets-fan-out-to-local-path-destinations) |
