...
**Two older requirements stay unmet, and are named here rather than left to look satisfied by a ceremony that skips them.** FR-KIT-004 requires a recovery kit generated during first-run setup with confirmation it has been saved; `RecoveryKitFactory` is still reachable only from the CLI's `kit export` verb. FR-SNP-007 requires first run to warn when the only destination shares a failure domain with the source; the console shows the failure domain per destination and issues no such warning. Both were scoped out of ADR-0044 deliberately — the passphrase is the step whose absence stops the product — and the ceremony they were written against now exists to hold them.
