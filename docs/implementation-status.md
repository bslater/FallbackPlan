...
**Call-site coverage is partial, and is counted rather than described.** Phase 2 declared 107 `[LoggerMessage]` messages and wired far fewer call sites than "the abstraction is threaded through every library" implies. The publish-and-restore path is now wired end to end — the scanner's roots and directories, the per-file captured/unchanged/failed records, and the per-item restore outcomes, all carrying `LogPath` — which is what makes `LogPrivacyTests` prove something rather than pass over an empty capture. **46 of 107 are called; the remaining 61 are frozen in `ArchitectureTests/LoggingShapeTests`** as a register that may shrink and must never grow: a new declaration fails the build without its call site, and wiring an old one fails until its line is deleted. Storage, catalogue, index, protocol, replication, retention and the scheduler still declare more than they emit.
