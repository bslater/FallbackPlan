...
Built. A service with no passphrase now says so on `describe_service`, and the first client to connect walks the operator through choosing one. What made a passphrase-only ceremony possible is that the passphrase was never per-set: `WriteOnlyDerivation` takes no repository identifier, so one `(passphrase, salt, params)` triple stamps every archive an installation will ever create. Setup provisions the installation; each set's staging archive is created from that credential on its first backup, replacing the silent format-1 fallthrough in `ServiceRuntime.ArchiveForAsync`.

Contract 1.13 carries `provision_installation` — the same sealed-envelope shape as ADR-0042's per-set verb, which keeps its own job as the adoption ceremony — and the setup state on `describe_service`. `CallerScope` is new and is the fact the code was missing: one handler served both listeners and `RemoteBindingState` said only whether the remote binding was on. The host now builds one handler per listener, and the pending ADR-0043 diagnostics work reuses it rather than inventing a second mechanism. Q14 is answered: a floor of twelve plus a modest estimate, enforced where a passphrase is chosen and never in `Passphrase.Create`, which is on the restore path.

**Both gaps that ADR-0044 scoped out are now being closed** (see the ADR-0013 and ADR-0044 amendments). FR-KIT-004's blocker turned out to be the kit *format* rather than the product: a kit demanded a repository id, which demanded an archive, a set and a destination. Kit format v2 drops it — an installation's keys derive from the passphrase alone — so the kit is generated in the ceremony that already holds it, and setup stays in a new `kit_required` state until the operator confirms saving it. FR-SNP-007's warning lands on `validate_set_draft`, which the console already calls live while editing, rather than in a passphrase ceremony that never sees a destination.

**Three requirements remain unmet and now say so in their own traceability rows** rather than sheltering under a range row that read as coverage for all six. FR-KIT-003: the transcribable text form is built and fixtured, the QR half is not. FR-KIT-005: setup records a durable confirmation and nothing more; never-generated/saved/stale as a continuously surfaced status is unbuilt, and whether an installation kit carrying no destinations can go stale at all is itself undecided.
