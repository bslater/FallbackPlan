...
| [0034](adr/0034-hub-and-spoke-destinations.md) | Hub-and-spoke destinations | **Partly built** | `FallbackPlan.Replication`, `Agent/FanOut`, `Application/DestinationSyncStore` · `Repository.Tests/EndToEnd/AgentPassTests`, `InterruptionTests/StoreCopyOrderTests` · [notes](#0034--sets-fan-out-to-local-path-destinations) |
...
What is left is destination verification (spec 04), hub-planned retention against a spoke (spec 06), and the console features gated on the two open questions of ADR-0028 — streaming restored content to the operator (Q18) and per-operator identity on a shared console (Q19).
...
### 0034 — sets fan out to local-path destinations

Three arc slices are built. **Configuration schema v2** (`Application/ClientConfiguration.cs`, `Application/DestinationConfiguration.cs`): named destinations (the cloud kinds schema-reserved), per-set destination references with optional retention overrides; refuses a destination-less set, a dangling reference, and the v1 schema — the last with the migration in the message. Pinned by `ClientConfigurationTests`.
...
**The status matrix** (`Application/StatusModel.cs`, the ADR-0027 amendment made real): the derivation's input is per set, per destination — sync state from the ledger, kind from the configuration, failure domain by device-identity comparison for a local path (the PT-8 placeholder finally replaced) — and the single-destination booleans are gone. `Protected`/`Verified` are earned only by an in-sync destination outside the source's failure domain; one laggard becomes a warning naming it, every supported destination behind or unreachable is `Degraded`, a reserved kind is a stated incapacity that never manufactures one. The command surface carries the matrix rows under each set's roll-up and the CLI renders them; `ApplicationServiceTests` pins the rollup table, `ServiceTests` pins the rows.

**Retention has begun with its pure core** (`FallbackPlan.Retention`, the architecture 11 placeholder activated): `RetentionPlanner` evaluates a set's policy against snapshot capture times and selects with stated reasons — an absent rule keeps everything, `min_generations` is the floor the other rules cannot override — and `ReplicationGate` holds any policy-expired snapshot a configured destination has not received, comparing snapshot publication generations to the generation each sync recorded at its start (`destinations.json` now carries it), never a clock; a laggard beyond the configured `deferral_days` turns the quiet hold into a warning (FR-GC-009). Nothing deletes yet: the marker, the tombstoned sweep and the `retention` verbs are the arc's next slice, and the planner-selects/collector-deletes separation (FR-GC-001) is already the shape of the code.

Still ahead: the collector (mark, tombstoned sweep, the `retention` verbs), the `sync` verb superseding `replicate` — sequenced in the [roadmap](roadmap.md). Quotas are enforced with the peer slices — see [0030](#0030--the-socket-exists). The negotiated pairing role and the termination notices landed with the peer slices — either side's ending now surfaces durably on both ends, and a fan-out refused `revoked` raises the notice itself — see [0030](#0030--the-socket-exists).
...
| [Hub-and-spoke arc](roadmap.md#the-hub-and-spoke-arc--multi-destination-backup-sets-current) | In progress ([ADR-0034](adr/0034-hub-and-spoke-destinations.md)): configuration schema v2, per-set staging archives, local-path and peer fan-out, the status matrix, termination notices and quota enforcement built; retention next — see [0034](#0034--sets-fan-out-to-local-path-destinations) |
