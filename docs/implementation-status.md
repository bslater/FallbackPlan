...
Still ahead: the per-destination status matrix (status still evaluates a single-destination placeholder), peer fan-out, termination notices, quotas, retention — sequenced in the [roadmap](roadmap.md).
...
| [Hub-and-spoke arc](roadmap.md#the-hub-and-spoke-arc--multi-destination-backup-sets-current) | In progress ([ADR-0034](adr/0034-hub-and-spoke-destinations.md)): configuration schema v2, per-set staging archives and local-path fan-out built; the status matrix next — see [0034](#0034--sets-fan-out-to-local-path-destinations) |
