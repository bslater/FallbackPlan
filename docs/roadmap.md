...
**Exit criteria (peer-to-peer):** the source can be destroyed and restored using only destination plus recovery kit — **destroyed meaning its durable local state as well as its archives**, so the recovering machine is a new device identity and reaches its replica by claiming it ([ADR-0046](adr/0046-replica-claim-after-total-loss.md), FR-DR-001) · no relay required on a LAN · no destination plaintext visibility · multi-day disconnection and resumption tested · quota exhaustion handled distinctly from disk-full · verify-on-reuse prevents a hostile writer corrupting another device's backup.

> The qualification is new, and it is a tightening rather than an addition. The criterion was already met under the reading that "destroyed" meant the archives: `PeerRetrievalTests` and `AlternateSiteTests` delete the staging archive and restore from the peer. Both keep the state directory, so the device keypair and the pairing survive the disaster — which is not the disaster. Total loss takes the identity too, and until the claim ceremony is built, that case cannot be recovered at all. Stated here because an exit criterion read generously is how a gap survives a phase review.
