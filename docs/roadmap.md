...
**Status:** all eleven slices are built and proven, the stretch trim included, and the operator surface landed with them — `sync` superseded `replicate`, and `retention` is commandable from a paired console ([implementation status](implementation-status.md#0034--sets-fan-out-to-local-path-destinations)). Destination verification ([spec 04](../specifications/peer-protocol/README.md#documents)) remains the Phase-2 tail outside this arc.
