...
**Status:** the service boundary is built on the local binding — writer-role exclusion, the command contract and its versioning, status aggregation, unlock, per-job progress, and a CLI that asks a running service to run a backup and takes direct mode when none is listening. **The remote binding is not built:** it validates and binds nothing, because pairing reuses [architecture 09 §3](architecture/09-replication-and-peers.md#3-pairing)'s machinery and that does not exist yet, and it is gated on [Q18](open-questions.md#q18--streaming-restored-content-to-a-remote-client) and [Q19](open-questions.md#q19--console-identity-and-multi-operator-access), both open. Two of the exit criteria below need it — the unpaired-client refusal and the no-plaintext-across-the-remote-binding claim — so those two are unmet by construction rather than untested, along with topologies 3 and 4 of [ADR-0028 §1](adr/0028-service-boundary-and-deployment-topologies.md). Restore, verify and check over the command surface are answered with a stated refusal naming them as read paths, not a silence.
