...
## The hub-and-spoke arc — multi-destination backup sets *(current)*
...
11. *(stretch)* **Staging trim** — once every destination holds an object durably, staging may drop it; the no-local-destination configuration stops costing two local copies.
