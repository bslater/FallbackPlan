...
**Status:** built and proven end to end — the flag stays **off by default** until the remaining tail lands: the peer write adapter (a peer destination is a stated `NotSupported` in the ledger for direct-ship), a retention-with-trimming drill on aged direct-ship snapshots, and then the default flip that retires the staging machinery.
